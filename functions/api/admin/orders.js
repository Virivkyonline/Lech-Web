...
export async function onRequestGet({ request, env }) {
  try {
    const store = kv(env);
    if (!store) {
      return j({ success: false, error: "Chýba KV binding LECHWEB_KV." }, 500);
    }

    if (!adminOk(request, env)) {
      return j({ success: false, error: "Nesprávny admin PIN." }, 401);
    }
    ...
    const ids = (await getJson(store, indexKey)) || [];
    ...
    return j({
      success: false,
      error: "Načítanie objednávok zlyhalo.",
